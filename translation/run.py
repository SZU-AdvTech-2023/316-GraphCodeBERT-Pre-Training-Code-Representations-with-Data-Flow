...
def extract_dataflow(code, parser, lang):
    ...
    try:
        ...
        index_to_code = {}
        for idx, (index, code) in enumerate(
                zip(tokens_index, code_tokens)):  # tokens_index[(start_point,end_point)....]
...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in enumerate(tqdm(examples, total=len(examples))):
        # extract data flow
        # code token还是还是code token 就是 dfg不知道是什么鬼
        ...
        # x[1],第x个variable在经过tokenize之前所在序列的序号,因为ori2cur_pos只是统计对应code token 经过tokenize后位置
    ...
    def __getitem__(self, item):
        ...
        for idx, (a, b) in enumerate(self.examples[item].dfg_to_code):
            if a < node_index and b < node_index:
                # 根据position_idx，从node_index这个位置开始才是variable的位置
                # code-variable 是双向的关系
                attn_mask[idx + node_index, a:b] = True
                attn_mask[a:b, idx + node_index] = True
        # nodes attend to adjacent nodes
        for idx, nodes in enumerate(self.examples[item].dfg_to_dfg):
            for a in nodes:
                # variable-variable的关系是单向的
...
def main():
    ...
    if args.do_train:
        ...
        tempers = torch.rand(1, requires_grad=True, device=device)
        for epoch in range(args.num_train_epochs):
            bar = tqdm(train_dataloader, total=len(train_dataloader))
            for batch in bar:
                batch = tuple(t.to(device) for t in batch)
                source_ids, source_mask, position_idx, att_mask, target_ids, target_mask = batch
                loss, _, _ = model(tempers, source_ids, source_mask, position_idx, att_mask, target_ids, target_mask)
                ...
                if (nb_tr_steps + 1) % args.gradient_accumulation_steps == 0:
                    # Update parameters
                    tempers.data.add_(-(args.learning_rate * tempers.grad.data))
                    optimizer.step()
            ...
            if args.do_eval and epoch in [int(args.num_train_epochs * (i + 1) // 20) for i in range(20)]:
                ...
                for batch in eval_dataloader:
                    ...
                    source_ids, source_mask, position_idx, att_mask, target_ids, target_mask = batch
                    with torch.no_grad():
                        _, loss, num = model(tempers, source_ids, source_mask, position_idx, att_mask, target_ids,
                                             target_mask)
                ...
                for batch in eval_dataloader:
                    ...
                    source_ids, source_mask, position_idx, att_mask, target_ids, target_mask = batch
                    with torch.no_grad():
                        preds = model(tempers, source_ids, source_mask, position_idx, att_mask)
                        # 没传target_ids，所以不会计算loss
                        for pred in preds:
                            t = pred[0].cpu().numpy()
                            t = list(t)
                            if 0 in t:
                ...
                if dev_bleu + xmatch > best_bleu:
                    ...
                    output_model_file = os.path.join(output_dir, "pytorch_model.bin")
                    torch.save(model_to_save.state_dict(), output_model_file)
                    output_tempers_file = os.path.join(output_dir, "tempers.pt")
                    torch.save(tempers, output_tempers_file)
            print("tempers:", tempers)
    output_dir = os.path.join(args.output_dir, 'checkpoint-best-bleu')
    output_tempers_file = os.path.join(output_dir, "tempers.pt")
    tempers = torch.load(output_tempers_file)
    if args.do_test:
        files = []
